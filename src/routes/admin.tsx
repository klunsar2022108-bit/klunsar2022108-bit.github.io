    ...
    <div className="mx-auto max-w-6xl px-4 py-14">
          ...
          <CardContent className="space-y-3">
            ...
            >
              <select
                ...
                onChange={(event) =>
                  setAllocationForm((current) => ({ ...current, enrollmentId: event.target.value }))
                }
          ...
          <CardContent>
            <form onSubmit={saveContent} className="space-y-3">
              ...
              <div className="rounded-md border border-dashed border-border bg-secondary/20 p-3 text-xs text-muted-foreground">
                Current key: <span className="font-medium text-foreground">{contentForm.key}</span>{" "}
                · Preview: {site?.[contentForm.key] ?? "No saved value yet"}
              </div>
              <textarea
        ...
        <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {users.map((user) => (
            <div key={user.id} className="rounded-lg border border-border bg-background p-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">{user.username ?? "No username"}</p>
      ...
      <div className="mt-10 rounded-2xl border border-border bg-primary px-6 py-10 text-primary-foreground">
        ...
        <p className="mt-3 max-w-2xl text-sm opacity-90">
          Use the admin tools to keep the website, classes and student information aligned with the
          real centre operations.
        </p>
