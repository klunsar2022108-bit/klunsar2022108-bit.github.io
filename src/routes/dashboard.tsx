...
export function DashboardPage() {
  ...
  const { data: exams = [] } = useQuery({ ...examsQuery, enabled: isStudent });
  const { data: orders = [] } = useQuery({
    ...userOrdersQuery(user?.id),
  ...
  const { data: adminInsightData } = useQuery({
    queryKey: ["dashboard-admin-insights", activeRole],
    enabled: isAdmin || isSuperAdmin,
    queryFn: async () => {
      const [ordersResult, profilesResult, paymentsResult] = await Promise.all([
        supabase
          .from("orders")
          .select(
            "id, customer_name, total, amount_paid, balance_due, status, payment_status, created_at",
          )
          .order("created_at", { ascending: false })
          .limit(8),
        supabase
        ...
        supabase
          .from("order_payments")
          .select("id, order_id, amount_paid, status, created_at")
          .order("created_at", { ascending: false })
          .limit(8),
      ]);
  ...
  const selectedChatMessages = selectedMetricAnalysis
    ...
    : [];
  const selectedInsightRows =
    isAdmin || isSuperAdmin
      ? selectedMetricData?.label === "Sales total" ||
        selectedMetricData?.label === "Total system revenue"
        ? (adminInsightData?.orders ?? [])
        : selectedMetricData?.label === "Pending approvals"
          ? (adminInsightData?.approvals ?? [])
          : selectedMetricData?.label === "Registered users" ||
              selectedMetricData?.label === "All user accounts"
            ? (adminInsightData?.approvals ?? [])
            : (adminInsightData?.payments ?? [])
      : [];
                ...
                {(isAdmin || isSuperAdmin) && (
                  <div className="rounded-xl border border-border bg-background p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Badge variant="secondary">{selectedInsightRows.length} records</Badge>
                    </div>
                    <div className="mt-3 space-y-2">
                      {selectedInsightRows.length === 0 ? (
                        <p className="text-sm text-muted-foreground">No matching records yet.</p>
                      ) : (
                        selectedInsightRows.slice(0, 6).map((row) => {
                          const record = row as Record<string, unknown>;
                          const label = String(
                            record.customer_name ?? record.full_name ?? record.email ?? record.id,
                          );
                          const value =
                            record.total != null
                              ? `Le ${Number(record.total).toLocaleString()}`
                              : String(record.status ?? record.approval_status ?? "Recorded");
                          return (
                            <div
                            ...
                            >
                              ...
                              <span className="shrink-0 font-semibold text-primary">{value}</span>
                            </div>
                          );
                        })
                      )}
            ...
            <Card className="mt-10 border-amber-200 bg-amber-50">
              <CardContent className="py-10 text-center">
                ...
                <p className="mx-auto mt-2 max-w-xl text-sm text-amber-900">
                  Your profile is awaiting admin approval. Student assignments, results, salary
                  review and teaching tools will unlock after approval.
                </p>
                  ...
                  <p className="font-display text-3xl font-bold text-blue-600 mt-2">
                    {
                      recentOrders.filter((o) =>
                        ["processing", "shipped"].includes(String(o.status)),
                      ).length
                    }
                  </p>
                </CardContent>
            ...
            <Card className="border-border/70">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Track your orders from submission through delivery. Payment must be approved
                ...
                <CardContent className="flex items-start gap-3 py-5">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-amber-800">
                      Profile updates, programme browsing, payment status and support remain
                      available while the centre reviews your registration.
                    </p>
                  </div>
                </CardContent>
              </Card>
                      ...
                      {recentOrders.some((order) => String(order.status) === "rejected") && (
                        <div className="rounded-lg border border-red-200 bg-red-50 p-3">
                          <p className="font-medium text-red-900">
                            Payment or registration rejected
                          </p>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <Card className="border-border/70">
            ...
            <CardContent className="space-y-3">
              {recentOrders.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No saved orders yet. Your enrolments and purchases will appear here after
                  checkout.
                </p>
              ) : (
                  ...
                  recentOrders
                    ...
                    .map((order) => (
                      ...
                      >
                        ...
                        <p className="text-xs">
                          Expected delivery:{" "}
                          {String(order.expected_delivery_at ?? "To be confirmed")}
                        </p>
